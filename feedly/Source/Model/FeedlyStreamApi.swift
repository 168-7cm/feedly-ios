...
protocol FeedlyStreamApiProtocol {
    func apiRequest<T: Codable>(access_token: String, continuation: String?) -> Single<T>
}

final class FeedlyStreamApi: FeedlyStreamApiProtocol {
    ...
    private lazy var access_token = try? Keychain(service: self.identifier).get("access_token")
